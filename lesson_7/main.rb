...
class Main
  ...
  def trains_by_station
    station = select_station
    block = lambda { |train| puts "Train #{train.train_id} : #{train.type}, #{train.wagons.size}" }
  ...
  private
    ...
    def create_block(type)
      if type == "cargo"
        lambda { |wagon| puts "Wagon #{wagon.number} : #{wagon.type}, #{wagon.remaining_volume}, #{wagon.taken_volume}" }
      else
        lambda { |wagon| puts "Wagon #{wagon.number} : #{wagon.type}, #{wagon.remaining_seats}, #{wagon.taken_seats}" }
      end
    end
