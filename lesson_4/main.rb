...
class Main
  ...
  def create_train
    ...
    type = gets.chomp
    @trains.push type == "passenger" ? PassengerTrain.new(train_id) : CargoTrain.new(train_id)
  end

  def trains
    @trains.each { |train| puts train.train_id }
  end
  ...
  private
    ...
    def select_station
      puts "Enter station`s title: "
      title = gets.chomp
      @stations.select { |station| station.title == title }[0]
    end
    ...
    def select_train
      puts "Enter train id: "
      train_id = gets.chomp
      @trains.select { |train| train.train_id == train_id }[0]
    end
